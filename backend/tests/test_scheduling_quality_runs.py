...
async def test_create_and_get_run_snapshots_current_report(client: AsyncClient, project: Project, live_period: Period):
    ...
    create_resp = await client.post("/api/v1/scheduling-quality-runs/", json={
        "period_id": str(live_period.id), "name": "Before cleanup",
    })
...
async def test_list_returns_summary_with_counts(client: AsyncClient, project: Project, live_period: Period):
    await client.post("/api/v1/scheduling-quality-runs/", json={"period_id": str(live_period.id), "name": "Snap A"})

    listing = (await client.get("/api/v1/scheduling-quality-runs/", params={"period_id": str(live_period.id)})).json()
...
async def test_delete_run(client: AsyncClient, project: Project, live_period: Period):
    create_resp = await client.post("/api/v1/scheduling-quality-runs/", json={"period_id": str(live_period.id), "name": "Temp"})
