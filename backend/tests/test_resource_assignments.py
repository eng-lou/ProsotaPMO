...
async def test_labour_assignment_costed_by_duration_and_utilisation(
    ...
    activity = await _create_activity(client, project, live_period, "Piling", duration_hours=40)  # 5 days at 8h/day
...
async def test_material_assignment_requires_quantity(client: AsyncClient, project: Project, live_period: Period):
...
async def test_material_assignment_costed_by_quantity(client: AsyncClient, project: Project, live_period: Period):
    activity = await _create_activity(client, project, live_period, "Piling")
    resource = await _create_resource(client, project, resource_type="material", name="CFA Piles", unit="nr", rate="576")
...
async def test_subcontractor_lump_sum_assignment(client: AsyncClient, project: Project, live_period: Period):
    activity = await _create_activity(client, project, live_period, "Piling")
...
async def test_list_assignments_by_activity(client: AsyncClient, project: Project, live_period: Period):
    activity = await _create_activity(client, project, live_period, "Piling")
    other_activity = await _create_activity(client, project, live_period, "Groundworks")
...
async def test_list_assignments_by_period(client: AsyncClient, project: Project, live_period: Period):
    activity = await _create_activity(client, project, live_period, "Piling")
    other_activity = await _create_activity(client, project, live_period, "Groundworks")
    r1 = await _create_resource(client, project, name="J. Davies")
    r2 = await _create_resource(client, project, name="360 Excavator", resource_type="equipment")
    ...
    resp = await client.get("/api/v1/resource-assignments/", params={"period_id": str(live_period.id)})
    assert resp.status_code == 200
...
async def test_update_utilisation_recomputes_budget(client: AsyncClient, project: Project, live_period: Period):
...
async def test_delete_assignment(client: AsyncClient, project: Project, live_period: Period):
    activity = await _create_activity(client, project, live_period, "Piling")
...
async def test_resource_from_other_project_rejected(
    client: AsyncClient, db, project: Project, live_period: Period, org
):
...
async def test_cannot_delete_resource_in_use(client: AsyncClient, project: Project, live_period: Period):
    activity = await _create_activity(client, project, live_period, "Piling")
    resource = await _create_resource(client, project)
...
async def test_assignment_rejects_frozen_period(client: AsyncClient, db, project: Project, frozen_period: Period):
    from app.models.activity import Activity

    activity = Activity(project_id=project.id, period_id=frozen_period.id, task_name="Frozen", code="ACT-9101")
    db.add(activity)
    await db.commit()
