...
async def test_quality_report_empty_period(client: AsyncClient, project: Project, live_schedule_period: SchedulePeriod):
    resp = await client.get("/api/v1/scheduling-quality/", params={"schedule_period_id": str(live_schedule_period.id)})
...
async def test_fully_linked_chain_passes_logic_checks(
    ...
    a = await _create_activity(client, project, live_schedule_period, "A", duration_hours=40)
    b = await _create_activity(client, project, live_schedule_period, "B", duration_hours=40)
...
async def test_hard_constraint_detected(client: AsyncClient, db: AsyncSession, project: Project, live_schedule_period: SchedulePeriod):
    await _anchor(db, live_schedule_period)
    await _create_activity(client, project, live_schedule_period, "A", duration_hours=40)
    await _create_activity(
        client, project, live_schedule_period, "B", duration_hours=40,
...
async def test_high_duration_flagged(client: AsyncClient, db: AsyncSession, project: Project, live_schedule_period: SchedulePeriod):
    await _anchor(db, live_schedule_period)
    await _create_activity(client, project, live_schedule_period, "Long haul", duration_hours=400)
...
async def test_negative_float_detected(client: AsyncClient, db: AsyncSession, project: Project, live_schedule_period: SchedulePeriod):
    await _anchor(db, live_schedule_period)
...
async def test_missed_task_detected(client: AsyncClient, db: AsyncSession, project: Project, live_schedule_period: SchedulePeriod):
    await _anchor(db, live_schedule_period)
    a = await _create_activity(client, project, live_schedule_period, "Excavation", duration_hours=40)
