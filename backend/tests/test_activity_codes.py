...
async def test_new_task_gets_t_prefix(client: AsyncClient, project: Project, live_schedule_period: SchedulePeriod):
    a = await _create(client, project, live_schedule_period, task_name="Excavation")
    assert a["code"] == "T-0001"
    assert a["wbs_role"] == "T"


async def test_new_finish_milestone_gets_m_prefix(client: AsyncClient, project: Project, live_schedule_period: SchedulePeriod):
    m = await _create(client, project, live_schedule_period, task_name="Practical Completion", activity_type="finish_milestone")
    assert m["code"] == "M-0001"
    assert m["wbs_role"] == "M"


async def test_new_start_milestone_gets_m_prefix(client: AsyncClient, project: Project, live_schedule_period: SchedulePeriod):
    # Both milestone kinds share the same "M" structural role — code
    # numbering doesn't distinguish start_milestone from finish_milestone.
    m = await _create(client, project, live_schedule_period, task_name="Mobilisation Complete", activity_type="start_milestone")
    assert m["code"] == "M-0001"
    assert m["wbs_role"] == "M"
...
async def test_indent_promotes_top_level_parent_to_p_prefix(
    ...
    await _create(client, project, live_schedule_period, task_name="Design", parent_id=parent["id"])
...
async def test_indent_promotes_nested_parent_to_w_prefix(
    ...
    grandparent = await _create(client, project, live_schedule_period, task_name="Programme")
    parent = await _create(client, project, live_schedule_period, task_name="Development", parent_id=grandparent["id"])
    await _create(client, project, live_schedule_period, task_name="Design", parent_id=parent["id"])
...
async def test_role_counters_are_independent_per_prefix(
    ...
    for i in range(5):
        await _create(client, project, live_schedule_period, task_name=f"Task {i}")
...
async def test_manual_code_rename_logged_and_not_reverted_by_recompute(
    ...
    await _create(client, project, live_schedule_period, task_name="Something else")
