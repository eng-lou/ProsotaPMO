# Vendored from https://github.com/VaclavNezerka/Cloud2BIM (MIT license,
# see LICENSE.cloud2bim in this directory) — 2026-08-20, run as a
# subprocess by app/services/cloud2bim_convert.py (`python cloud2entities.py
# <config path>`), one call per SiteCapture "Generate IFC" request, with
# its own working directory set to a fresh per-request temp dir (this
# script writes scratch/log files to plain relative paths like
# "output_xyz/", "images/", "log.txt" — never parameterized in the
# original, so real isolation across concurrent requests comes entirely
# from that per-request cwd, not from anything in this file). Only real
# change from upstream: plot_segmented_plane flipped to False just below
# (aux_functions.py's own header explains why open3d — the only thing that
# flag ever touched — was removed entirely, not just disabled).
...
slabs, horizontal_surface_planes = identify_slabs(points_xyz, points_rgb, bfs_thickness,
                                                  ...
                                                  plot_segmented_plane=False)  # PROSOTA PATCH — was True; that flag only ever gated the now-removed open3d preview window
...
# PROSOTA PATCH — moved up from inside the storey loop below, where they
# ran AFTER identify_walls() rather than before it. identify_walls() (via
# plotting_functions.py's plot_binary_image) unconditionally saves to
# images/pdf/wall_mask.pdf with no flag to skip it — a real bug in the
# original ordering that only worked upstream because their own repo
# already ships an images/ directory from prior runs (confirmed: a fresh
# directory reproduces FileNotFoundError: 'images/pdf/wall_mask.pdf').
...
for i, storey_pointcloud in enumerate(point_cloud_storeys):
    ...
    for j in range(len(start_points)):
        # PROSOTA PATCH (2026-08-21) — confirmed against a real
        # 232M-point/15mm-resolution multi-wing scan from Maro:
        # identify_walls() can hand back a wall whose start/end point is
        # NaN or coincident (the same degenerate-wall case
        # space_generator.py's own identify_zones already guards against —
        # see that file's _is_degenerate_wall). Left unfiltered here, it
        # reaches generate_ifc.py's wall_axis_placement() and crashes
        # ifcopenshell with "RuntimeError: Only finite values are
        # allowed" when building the IfcCartesianPoint. Skipping the
        # phantom wall (and its opening detection, meaningless for a wall
        # that was never real) is the same fix as before, just applied at
        # this second, independent consumer of the raw walls list.
        (sx, sy), (ex, ey) = start_points[j], end_points[j]
        if any(math.isnan(v) for v in (sx, sy, ex, ey)) or math.hypot(ex - sx, ey - sy) < 1e-9:
            print(f"Warning: skipping degenerate wall (storey {i + 1}, index {j}) — "
                  f"start {start_points[j]}, end {end_points[j]}.")
            continue
