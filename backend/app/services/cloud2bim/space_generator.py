...
def plot_wall_center_lines(walls_dictionary, title):
    ...
    plt.rc('text', usetex=False)  # PROSOTA PATCH — was True; no LaTeX install on this server, and matplotlib's own mathtext already renders the $...$ labels used here without it
...
def process_disconnected_walls(walls_for_processing, not_connected_walls, min_wall_length):
    i = 0
    iterations = 0
    # PROSOTA PATCH (2026-08-21) — diagnostic instrumentation + a hard
    # safety cap, added after a real run (18GB memory, 60+ min, killed
    # manually) blew far past what the earlier restart-loop fix should
    # have allowed. flush=True because this process's stdout is fully
    # buffered when redirected to a file — without it, none of this ever
    # reaches disk until the process exits (or never, if it hangs).
    while i < len(walls_for_processing):
        iterations += 1
        if iterations % 200 == 0:
            print(f"process_disconnected_walls: iteration {iterations}, i={i}, "
                  f"walls={len(walls_for_processing)}", flush=True)
        if iterations > 200_000:
            print(f"Warning: process_disconnected_walls exceeded 200000 iterations "
                  f"(walls={len(walls_for_processing)}) — aborting with current state "
                  f"rather than running unbounded.", flush=True)
            break
        ...
        for not_connected_wall in not_connected_walls:
            ...
            # PROSOTA PATCH (2026-08-21) — was `i = 0; break`. Both
            # divisions above replace walls_for_processing[i] in place
            # with its two new pieces at i, i+1 — nothing before i
            # changed, so there is nothing earlier to re-scan for. `i`
            # already points at the first new piece, and the `if not
            # divided: i += 1` guard below leaves it untouched, so the
            # very next while-iteration naturally re-examines it anyway.
            # Confirmed against a real storey the earlier slab-thickness
            # fix restored full height to: the unconditional restart made
            # this function's cost scale roughly cubically with wall
            # count — a storey with ~72 walls finished in seconds, but one
            # with a normal ~200+ real walls (once given its real height
            # back) never finished in 50+ minutes. This preserves the
            # exact same splitting behaviour without the wasted rescans.
            if need_restart:
                break
                ...
                if i != j:
                    if is_point_on_centerline(wall['start_point'], other_wall):
                        ...
                        if new_wall_1 and new_wall_2:
                            ...
                            # PROSOTA PATCH — was `i = 0`. other_wall (at
                            # j != i) was split, not wall (at i) itself —
                            # wall's own object is untouched, only its
                            # position can shift, and only by exactly one
                            # slot, and only if the insertion landed
                            # before it.
                            if j < i:
                                i += 1
                            divided = True
...
def _is_degenerate_wall(wall):
    # PROSOTA PATCH (2026-08-21) — confirmed against a real 232M-point,
    # 15mm-resolution multi-wing scan from Maro: a wall whose start and
    # end point coincide (or contain NaN) crashes shapely's own
    # unary_union deep inside extract_space_dimensions with "Edge
    # direction cannot be determined because endpoints are equal".
    # aux_functions.py's own distance_point_to_line already detects this
    # exact case and warns ("Line start and end points are the same.
    # Returning NaN.") but never removes the offending wall, so it just
    # keeps poisoning every downstream comparison (is_point_on_centerline,
    # find_intersection, ...) with NaN until shapely hard-crashes on it.
    (x1, y1), (x2, y2) = wall['start_point'], wall['end_point']
...
def identify_zones(walls_local, snapping_distance=0.5, plot_zones=True):
    walls_local = [wall for wall in walls_local if not _is_degenerate_wall(wall)]
    ...
    # Re-filter (2026-08-21, same PROSOTA PATCH as above) — process_centerlines
    # can itself produce a new degenerate wall while splitting/extending
    # (e.g. dividing right at an existing endpoint), so the walls it hands
    # to extract_space_dimensions below need the same guard, not just the
    # walls that came in.
    updated_walls = [wall for wall in updated_walls if not _is_degenerate_wall(wall)]
