# Vendored from https://github.com/VaclavNezerka/Cloud2BIM (MIT license, see
# LICENSE.cloud2bim in this directory) — 2026-08-20, per Maro: "pointcloud to
# ifc" / "build". Patched for headless server use in two places (search
# "PROSOTA PATCH" in this file): open3d and the `e57` package are both
# genuinely unused by this app's own call path (Cloud2BIM only touches
# open3d inside optional-visualization branches this app always disables,
# and `e57` is only used for its own direct .e57 reading — this app always
# hands Cloud2BIM a plain .xyz instead, converted server-side beforehand by
# app/services/e57_convert.py's own pye57-based pipeline, already proven
# against a real 105-scan MatterPak export — see that module's own header).
# Confirmed by reading every o3d./e57. call site directly before removing
# these imports, not assumed: open3d's own PyPI wheels don't support this
# app's Python version anyway (3.14; open3d tops out at 3.12), so this
# removal is also what makes running Cloud2BIM in this app's own venv
# possible at all, without a second Python install.
import os
import sys
import time
import math
...
import matplotlib
matplotlib.use('Agg')  # PROSOTA PATCH — headless: no display, and Agg's own
# plt.show() is a documented no-op that never renders, so it never invokes
# the LaTeX toolchain plt.rc('text', usetex=False)  # PROSOTA PATCH — was True; no LaTeX install on this server elsewhere in this vendored
# code would otherwise need (confirmed no server here has a LaTeX install,
# and installing one just for unused debug chart labels isn't reasonable).
...
def load_selective_lines(filename, step):
    with open(filename, 'r') as file:
        # Skip the first line
        next(file)
        # PROSOTA PATCH — was split('\t'); every xyz file this app ever
        # produces (e57_convert.py's own server-side conversion, and the
        # frontend's own MatterPak-cloud.xyz upload path) is space-
        # delimited, not tab-delimited, so this always raised
        # "could not convert string to float" the moment dilution was
        # ever turned on. split() with no argument splits on any run of
        # whitespace (space OR tab), so this covers both.
...
# PROSOTA PATCH (2026-08-21) — confirmed against a real multi-wing scan
# from Maro with pitched roofs: a real floor/ceiling slab always measured
# ~270-300mm thick in that scan, but the storey right under a pitched
# roof measured 3220mm and 2997mm. A sloped roof surface spans a wide
# Z-range at high point density over the same footprint, so the
# elif-(i%2)==0 branch below (median-to-median thickness between two
# candidate z-bands) reads the whole roof slope as one giant "slab" —
# which then starves the storey underneath of wall height
# (wall_height = next_slab.bottom - (this_slab.bottom + this_slab.thickness)
# in cloud2entities.py), which is what made that storey's walls come out
# a few centimetres tall instead of a real storey height. This library has
# no pitched-roof support at all (nothing in this codebase mentions
# roof/pitch/slope) — capping thickness doesn't add that, it just stops a
# misread roof from silently deleting real wall height it never should
# have consumed.
MAX_SANE_SLAB_THICKNESS = 0.5
...
def identify_slabs(points_xyz, points_rgb, bottom_floor_slab_thickness, top_floor_ceiling_thickness,
    ...
    for i in range(len(h_surf_candidates)):
        if i == 0:
            ...
            x_coords, y_coords, polygon = create_hull_from_histogram(horiz_surface_planes[i], pc_resolution,
                                                                     grid_coefficient=5, plot_graphics=False,  # PROSOTA PATCH — was True; triggers matplotlib usetex=True LaTeX rendering this server has no LaTeX install for (see this file's own top-of-file header)
            ...
            if slab_thickness > MAX_SANE_SLAB_THICKNESS:
                # Roof-corrupted read (see MAX_SANE_SLAB_THICKNESS's own
                # comment above) — keep the correctly-detected top surface
                # as the anchor and clamp thickness, rather than trusting
                # a multi-metre "slab" no real floor/ceiling structure is.
                print('Warning: slab thickness %.1f mm at z=%.3f exceeds %.1f mm — '
                      'likely a pitched roof read as one surface, clamping.'
                      % (slab_thickness * 1000, slab_top_z_coord, MAX_SANE_SLAB_THICKNESS * 1000))
                slab_thickness = MAX_SANE_SLAB_THICKNESS
                slab_bottom_z_coord = slab_top_z_coord - slab_thickness
    ...
    # PROSOTA PATCH — the original open3d-based `plot_segmented_plane` preview
    # window is removed (see this file's own top-of-file header); this app
    # always calls identify_slabs with plot_segmented_plane=False, so the
    # branch was already dead for every real call, but the open3d import
    # itself no longer exists at all now.
...
# PROSOTA PATCH — visualize_segmented_pointclouds (an open3d live-preview
# window) removed: confirmed unreachable from cloud2entities.py's own real
# call graph before removing it (grepped for its call site — none exists),
# not just unused by this app's own config.
    ...
    try:
        ...
        for x_start, x_end in openings:
            ...
            if plot_histograms_for_openings:
                # Plotting
                plt.rc('text', usetex=False)  # PROSOTA PATCH — was True; no LaTeX install on this server
