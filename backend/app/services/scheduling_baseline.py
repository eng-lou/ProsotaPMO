...
async def set_baseline(db: AsyncSession, period_id: uuid.UUID) -> list[Activity]:
    """Snapshot every activity's current start/finish/duration_hours into
    bl_start/bl_finish/bl_duration_hours for this period.
    ...
    for a in activities:
        ...
        a.bl_duration_hours = a.duration_hours
        # Same formula as app/services/activity.py:_apply_computed_fields, but that
        # helper also resets total_float_hours/free_float_hours/is_critical to null — appropriate
