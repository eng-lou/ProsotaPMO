...
TOOLS: list[dict] = [
    ...
    # grid coordinates a human would immediately want to rearrange anyway.
    {
        ...
        "description": (
            ...
            "33 widget types accept an optional filter — the exact same {field, operator, "
            ...
            "exact name right matters. Widgets that share the same underlying record type share "
            "the same field list below, whether they render a table or a chart (a chart just "
            "groups/aggregates from the same rows a table would list). Any other widget_type "
            "(e.g. kpi_strip, dcma_score, project_narrative, camera_view_gallery — a numeric "
            "summary computed over the WHOLE project, or a genuinely different data source) must "
            "omit filter entirely.\n"
            "- top_risks, risk_register_table, risks_by_category, risks_by_owner, "
            "threats_vs_opportunities, response_strategy_breakdown, risk_ageing_table — fields on "
            "a Risk: id, code, title, category, area, status, risk_owner, risk_type "
            "('threat'|'opportunity'), response_strategy, rating, emv_cost, emv_schedule_days, "
            "date_raised. E.g. a specific risk: {field:'code', operator:'eq', value:'R-004'}; a "
            "category: {field:'category', operator:'eq', value:'Cost'}; risks over £50k EMV: "
            "{field:'emv_cost', operator:'gt', value:'50000'}.\n"
            "- cost_elements_table, cost_breakdown_by_group, cost_breakdown_by_owner, "
            "budget_utilisation, bac_vs_eac_by_group — fields on a Cost Element: id, code, "
            "description, element_group, cost_owner, status, bac, ac, pct_complete, cpi, eac, "
            "vac, plus any User Defined Field configured for entity_type='cost_element' (see "
            "udf. below).\n"
            "- resource_assignments_table, resource_budget_by_type, "
            "resource_budget_by_discipline, resource_budget_by_company, top_resources_by_budget "
            "— fields on a Resource Assignment: id, resource_name, resource_type "
            "('labour'|'equipment'|'material'|'subcontractor'|'cost'|'crew'), discipline, "
            "company, role, budget, activity_id, activity_task_name, plus any UDF configured for "
            "entity_type='resource'. A request for one NAMED resource (not a whole type) means "
            "{field:'resource_name', operator:'eq', value:'<its exact name>'} — resolve the "
            "exact name/type first via find_records(record_type='resource') or "
            "get_project_snapshot, never guessed or partially matched.\n"
            "- open_items_by_owner, issues_by_status, issues_ageing_table, "
            "decisions_pending_table, changes_by_ccb_decision — fields on an ICD item: id, code, "
            "title, item_type ('issue'|'change'|'decision'), status, priority, owner, "
            "raised_date, due_date, closed_date, severity, decision_maker, required_by, "
            "ccb_decision, cost_impact, schedule_impact_days.\n"
            "- baseline_variance_table, critical_activities_table, near_critical_watch_list, "
            "float_distribution, activities_by_category, activity_status — fields on a scheduled "
            "Activity: id, code, task_name, start, finish, bl_finish, variance_days, "
            "total_float_hours, is_critical, pct_complete, schedule_category, suspend_date, "
            "resume_date, wbs_path (a dotted path like '1.2.3' — {field:'wbs_path', "
            "operator:'starts_with', value:'1.2'} scopes to EVERYTHING under WBS node 1.2, the "
            "exact same convention Scheduling's own Filters/Highlights already use for this same "
            "field; a plain 'eq' would only ever match one exact activity), plus any UDF "
            "configured for entity_type='activity' (see udf. below). A specific activity: "
            "{field:'code', operator:'eq', value:'T-0074'}.\n"
            "- milestones_table, milestone_timeline — fields on a Milestone: id, task_name, "
            "finish, bl_finish, is_critical, variance_days (no UDF support — milestones aren't a "
            "UDF entity_type).\n"
            "- lookahead_planner — fields on a Look-Ahead item: id, code, task_name, start, "
            "finish, pct_complete, total_float_hours, is_critical, has_incomplete_predecessor "
            "(no UDF support).\n"
            "- mitigation_actions_table — fields on a Risk Mitigation Action: id, risk_id, "
            "risk_code, code, description, owner, due_date, status, pct_complete (no UDF "
            "support).\n"
