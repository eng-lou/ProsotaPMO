...
export function MitigationActions({ riskId }: MitigationActionsProps) {
  ...
  if (loading) return <p className="text-xs text-gray-400 dark:text-prosota-muted px-4 py-3">Loading mitigation actions…</p>

  return (
    <div className="px-4 py-3 bg-gray-50 dark:bg-prosota-panel2 border-t border-gray-100 dark:border-prosota-line">
      <div className="text-xs font-semibold text-gray-600 dark:text-prosota-muted mb-2">Mitigation actions</div>
      {error && <p className="text-xs text-red-600 dark:text-red-400 mb-2">{error}</p>}

      {actions.length === 0 && !adding && (
        <p className="text-xs text-gray-400 dark:text-prosota-muted mb-2">No mitigation actions yet.</p>
      )}
