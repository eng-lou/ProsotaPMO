...
export function UnloadModelDialog({ name, linkCount, keyframeCount, onUnloadOnly, onUnloadAndDelete, onCancel }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={onCancel}>
      <div className="w-96 bg-white rounded-lg shadow-xl border border-gray-200" onClick={e => e.stopPropagation()}>
        <div className="px-4 py-3 border-b border-gray-100">
          <h3 className="text-sm font-bold text-gray-800">Unload "{name}"</h3>
        </div>
        <div className="px-4 py-3 space-y-2">
          <p className="text-xs text-gray-700">This model still has {parts.join(' and ')}.</p>
        ...
        <div className="px-4 py-3 border-t border-gray-100 flex justify-end gap-2">
          ...
          <button onClick={onUnloadOnly} className="text-xs px-3 py-1.5 rounded-md border border-gray-300 bg-white text-gray-700 hover:bg-gray-50">
            Unload Only
          </button>
