  ...
  return (
    <div className="flex-1 flex flex-col overflow-y-auto">
      <div className="px-3 py-2 border-b border-gray-100 sticky top-0 bg-white">
        <span className="text-xs text-gray-500">Rigging</span>
      </div>
      ...
      <div className="px-3 py-2 space-y-1.5 border-b border-gray-100">
        {!childTarget ? (
          ...
          <div className="flex items-center gap-1.5 text-xs text-gray-600 bg-sky-50 border border-sky-100 rounded px-2 py-1">
            ...
            <button onClick={() => onClearParent(existingParent.id)} className="text-gray-400 hover:text-red-600 shrink-0">✕</button>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            <select
              ...
              className="flex-1 min-w-0 text-xs border border-gray-300 rounded px-1 py-1"
            >
            ...
            <button
              ...
              className="text-xs px-2 py-1 rounded border border-gray-300 text-gray-600 disabled:text-gray-300 disabled:border-gray-200 hover:bg-gray-50 disabled:hover:bg-transparent"
            >
              Set Parent
            </button>
          </div>
      ...
      {elementParents.length === 0 ? (
        ...
        <div className="divide-y divide-gray-100">
          {elementParents.map(ep => (
            <div key={ep.id} className="px-3 py-1.5 flex items-center gap-1.5 text-xs text-gray-600">
              <span className="flex-1 truncate">{ep.child_element_ref} → {ep.parent_element_ref}</span>
              <button onClick={() => onClearParent(ep.id)} title="Clear" className="text-gray-400 hover:text-red-600 shrink-0">✕</button>
            </div>
          ))}
        </div>
      )}
