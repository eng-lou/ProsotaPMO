...
interface Props {
  file: File
  kind: 'ifc' | 'mesh'
  onConfirm: (sourceUpAxis: UpAxis) => void
  onCancel: () => void
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={onCancel}>
      <div className="w-80 bg-white rounded-lg shadow-xl border border-gray-200" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-4 py-3 space-y-3">
          <div>
            <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-1">File</div>
            <div className="text-xs text-gray-700 truncate" title={file.name}>{file.name}</div>
            <div className="text-[11px] text-gray-400 mt-0.5">{kind === 'ifc' ? 'IFC model' : '3D mesh (GLTF/OBJ/FBX)'}</div>
          </div>
          <div>
        ...
        <div className="px-4 py-3 border-t border-gray-100 flex justify-end gap-2">
          ...
          <button onClick={() => onConfirm(upAxis)} className="text-xs px-3 py-1.5 rounded-md border border-gray-900 bg-gray-900 text-white hover:bg-gray-800">
            Import
          </button>
        </div>
      </div>
