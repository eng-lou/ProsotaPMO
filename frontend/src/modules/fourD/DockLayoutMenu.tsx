    ...
    <div ref={rootRef} className="relative">
      <button
        ...
        className={`text-xs px-2.5 py-1 rounded-md border font-medium ${
          open ? 'bg-gray-900 text-white border-gray-900' : 'bg-white dark:bg-prosota-panel text-gray-600 dark:text-prosota-muted border-gray-300 dark:border-prosota-line hover:bg-gray-50 dark:hover:bg-prosota-panel2'
        }`}
      >
        Layout ▾
      </button>
      {open && (
        <div className="absolute z-50 top-full left-0 mt-1 w-72 bg-white dark:bg-prosota-panel border border-gray-200 dark:border-prosota-line rounded-lg shadow-lg p-2">
          <div className="max-h-64 overflow-y-auto divide-y divide-gray-100">
            {loading && <p className="px-1 py-2 text-xs text-gray-400 dark:text-prosota-muted">Loading…</p>}
            {!loading && layouts.length === 0 && (
              <p className="px-1 py-2 text-xs text-gray-400 dark:text-prosota-muted">No saved layouts yet.</p>
            )}
            {layouts.map(layout => (
              <div key={layout.id} className="flex items-center gap-1.5 py-1.5">
                {renamingId === layout.id ? (
                  ...
                  <span className="flex-1 min-w-0 truncate text-xs text-gray-700 dark:text-prosota-muted" title={layout.name}>{layout.name}</span>
                )}
                {layout.is_active ? (
                  <span className="text-[10px] text-blue-600 dark:text-prosota-azure font-medium shrink-0">✓ Applied</span>
                ) : (
                  <button onClick={() => handleApply(layout)} title="Apply" className="text-xs text-gray-400 dark:text-prosota-muted hover:text-gray-700 dark:hover:text-prosota-paper shrink-0">Apply</button>
                )}
                <button
                ...
                <button onClick={() => handleDelete(layout)} title="Delete" className="text-xs text-gray-400 dark:text-prosota-muted hover:text-red-600 dark:hover:text-red-400 shrink-0">✕</button>
              </div>
            ))}
          </div>

          <div className="border-t border-gray-100 dark:border-prosota-line mt-1 pt-1.5">
            {savingName !== null ? (
              <div className="flex items-center gap-1.5">
                ...
                <button
                  ...
                  className="text-xs px-2 py-0.5 rounded border border-gray-300 dark:border-prosota-line bg-white dark:bg-prosota-panel text-gray-600 dark:text-prosota-muted hover:bg-gray-50 dark:hover:bg-prosota-panel2 disabled:opacity-50"
                >
                  Save
                </button>
              </div>
            ) : (
              <button onClick={() => setSavingName('')} className="w-full text-left text-xs text-gray-600 dark:text-prosota-muted hover:text-gray-900 px-1 py-1">
                + Save current as new layout
              </button>
            )}
            <button onClick={handleReset} className="w-full text-left text-[11px] text-gray-400 dark:text-prosota-muted hover:text-gray-600 dark:hover:text-prosota-paper px-1 pt-1">
              ↺ Reset to default
            </button>
          </div>
        </div>
