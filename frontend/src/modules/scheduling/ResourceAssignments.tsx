...
export function ResourceAssignments({ activity, resources, onChange }: Props) {
  ...
  if (loading) return <p className="text-xs text-gray-400 dark:text-prosota-muted px-4 py-3">Loading resources…</p>
  ...
  return (
    <div className="px-4 py-4 bg-gray-50 dark:bg-prosota-panel2 border-t border-gray-100 dark:border-prosota-line text-xs">
      <div className="text-xs font-semibold text-gray-500 dark:text-prosota-muted uppercase tracking-wide mb-1.5">Assigned Resources</div>
      {assignments.length === 0 ? (
        ...
        <table className="w-full text-xs border-collapse mb-2">
          ...
          <tbody>
            {assignments.map(a => (
              <tr key={a.id} className="border-t border-gray-100 dark:border-prosota-line bg-white dark:bg-prosota-panel">
                <td className="py-1 px-1">
                  {a.resource_name}
                  <span className="text-gray-400 dark:text-prosota-muted"> ({RESOURCE_TYPE_LABELS[a.resource_type]})</span>
                </td>
                {editingId === a.id ? (
                  <>
                    <td className="py-1 px-1">
                    ...
                    <td className="py-1 px-1 text-right">
                      {isTimeBased(a.resource_type) ? (
                        ...
                        <span className="text-gray-400 dark:text-prosota-muted">lump sum</span>
                      ) : (
                        <input
                          type="number" min={0} step={0.01}
                          value={editValues.quantity}
                          onChange={e => setEditValues(v => ({ ...v, quantity: e.target.value }))}
                          className="w-20 border border-blue-400 dark:bg-prosota-panel2 dark:text-prosota-paper rounded px-1 py-0.5 text-xs text-right"
                        />
                      )}
                    </td>
                    <td className="py-1 px-1 text-right font-medium text-gray-400 dark:text-prosota-muted">—</td>
                    <td className="py-1 px-1 text-right whitespace-nowrap">
                      <button onClick={() => handleSaveEdit(a)} className="text-blue-600 dark:text-prosota-azure hover:text-blue-700 dark:hover:text-prosota-cyan mr-1.5">Save</button>
                  ...
                  <>
                    <td className="py-1 px-1 text-gray-500 dark:text-prosota-muted">{a.role ?? '—'}</td>
                    <td className="py-1 px-1 text-right">
                    ...
                    <td className="py-1 px-1 text-right whitespace-nowrap">
                      <button onClick={() => startEdit(a)} className="text-blue-600 dark:text-prosota-azure hover:text-blue-700 dark:hover:text-prosota-cyan mr-1.5">Edit</button>
                      <button onClick={() => handleDelete(a)} className="text-gray-400 dark:text-prosota-muted hover:text-red-600 dark:hover:text-red-400">✕</button>
                    </td>
                  </>
                )}
              </tr>
            ))}
            <tr className="border-t border-gray-200 dark:border-prosota-line font-semibold bg-white dark:bg-prosota-panel">
            ...
            <tr className="bg-white dark:bg-prosota-panel">
              <td className="py-1 px-1 text-gray-500 dark:text-prosota-muted" colSpan={3}>
                Actual {actualsUnit === 'cost' ? 'Cost' : actualsUnit === 'hours' ? 'Hours' : 'Days'}
                <span className="text-gray-400 dark:text-prosota-muted"> — synced to the linked Cost Plan line</span>
              </td>
              <td className="py-1 px-1 text-right" colSpan={2}>
                <span className="inline-flex items-center gap-1.5 justify-end w-full">
                  {singleTimeBasedAssignment(assignments) && (
                    <span className="inline-flex items-center border border-gray-300 dark:border-prosota-line rounded overflow-hidden">
                      {(['cost', 'hours', 'days'] as const).map(u => (
                        <button
                          ...
                          className={`px-1.5 py-0.5 text-[10px] ${actualsUnit === u ? 'bg-gray-900 text-white' : 'bg-white dark:bg-prosota-panel text-gray-500 dark:text-prosota-muted hover:bg-gray-50 dark:hover:bg-prosota-panel2'}`}
                        >
                          {u === 'cost' ? '£' : u === 'hours' ? 'h' : 'd'}
                        </button>
                      ))}
                  ...
                  <input
                    ...
                    placeholder="0.00"
                    className="w-20 border border-gray-300 dark:border-prosota-line dark:bg-prosota-panel2 dark:text-prosota-paper rounded px-1 py-0.5 text-xs text-right disabled:opacity-50"
                  />
                </span>
              </td>
            </tr>
      ...
      {isParent ? (
        <div className="text-[11px] text-gray-400 dark:text-prosota-muted">Not applicable to a WBS/Project summary — assign resources to its individual tasks/milestones instead.</div>
      ) : adding ? (
        <div className="flex items-center gap-2 mb-2 flex-wrap">
          <select value={resourceId} onChange={e => setResourceId(e.target.value)} className="text-xs border border-gray-300 dark:border-prosota-line dark:bg-prosota-panel2 dark:text-prosota-paper rounded px-2 py-1">
          ...
          <input
            ...
            placeholder="Role (optional)"
            className="text-xs border border-gray-300 dark:border-prosota-line dark:bg-prosota-panel2 dark:text-prosota-paper rounded px-2 py-1 w-32"
          />
          {selectedResource && isTimeBased(selectedResource.resource_type) && (
            <label className="text-xs text-gray-600 dark:text-prosota-muted flex items-center gap-1">
              Utilisation
              <input
                type="number"
                min={1}
          ...
          {selectedResource && selectedResource.resource_type === 'material' && (
            <input
              ...
              className="text-xs border border-gray-300 dark:border-prosota-line dark:bg-prosota-panel2 dark:text-prosota-paper rounded px-2 py-1 w-24"
            />
          )}
          {selectedResource && selectedResource.resource_type === 'subcontractor' && (
            <span className="text-xs text-gray-400 dark:text-prosota-muted">Flat lump sum — no quantity needed</span>
          )}
          <button
            onClick={handleAdd}
        ...
        <button
          ...
          className="text-xs text-blue-600 dark:text-prosota-azure hover:text-blue-700 dark:hover:text-prosota-cyan font-medium disabled:text-gray-300 disabled:cursor-not-allowed"
        >
          + Assign Resource
        </button>
      )}
