...
export function RescheduleWidget({ period, onApplied, onClose }: Props) {
  ...
  // reschedules within one open session show the right "current" value without
  // depending on the parent re-fetching its Period object (see lib/usePeriod.ts,
  // which only fetches once on mount). previousDataDate captures what it was right
  // before the most recent apply, for the before/after summary line.
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-5 mb-4">
      ...
      <div className="flex items-center gap-2 mb-3 bg-gray-50 border border-gray-200 rounded-md px-3 py-2">
        ...
        <span className="text-xs text-gray-400 ml-1">
          — the anchor unconstrained activities can't start earlier than; this is what Reschedule actually moves.
        </span>
      </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-md px-3 py-2 text-xs text-amber-800 mb-3">
        Shifts the data date by {amount} {unit} and re-runs the critical path engine. Activities with a hard
